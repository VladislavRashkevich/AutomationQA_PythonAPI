...
class Assertions:
    ...
    def assert_json_has_not_keys(response: Response, names: list):
        try:
            ...
            assert False, f"Response is not in JSON format. Response text is '{response.text}'"
        for name in names:
            assert not (name in response_as_dict), f"Response JSON shouldn't have key '{name}'. But it is present"
    ...
    @staticmethod
    def assert_content(response: Response, expected_string, error_message):
        assert response.content.decode("utf-8") == expected_string, error_message
